...
def accidentes_por_distrito_tipo(datos):
    """
    Devuelve un contador con el número de accidentes que ha habido en cada distrito
...
def puntos_negros_distrito(datos, distrito, k):
    ...
    especificado, siendo cada tupla del estilo (lugar, numero_de_accidentes), 
    ordenadas de mayor a menor número de accidentes y, en caso de empate, alfabéticamente.
...
def codigos_postales(monumentos):
    ...
    ordenada por el número de monumentos en cada código postal de manera decreciente, y
    por el código postal en orden alfabético ascendente en caso de empate.
    """
...
def busqueda_distancia(monumentos, direccion, distancia):
    """
    Recibe una lista de monumentos, una dirección y una distancia y devuelve una lista de 
    tuplas (título, id_monumento, distancia) de aquellos monumentos que están a una distancia 
    menor o igual que la indicada desde la dirección dada. 

    La lista se ordena por la distancia desde la dirección dada de manera creciente. 

    La distancia se mide en kilómetros metros.
    """
    ...
    for m in monumentos:
        if 'location' in m and 'latitude' in m['location'] and 'longitude' in m['location']:
            ...
            calculo_distancia = distance(coords_original, coords_monumento).km
            if calculo_distancia < distancia:
                ternas.append((title, id_monumento, calculo_distancia))
    #Documentación de python ordenar tuplas
    ternas.sort(key=lambda t: t[2])
    return ternas
